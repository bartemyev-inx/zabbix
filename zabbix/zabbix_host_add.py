...
def get_options():
    ...
    \t [--status default 0 monitored] [--inventory_mode default 1]
"""
...
2 - (default) SNMP v1/2c.""")
    parser.add_option("--inventory_mode",action="store",type="int",\
        dest="inventory_mode",default="1",help="""Inventory mode of the host. 
Possible values are:
-1 - disable
0 - enable manual;
1 - (default) enable automatic.""")
...
1 - unmonitored host.""")
    parser.add_option("-f","--file",dest="filename",\
        metavar="FILE",help="Load values from input file. Specify - for standard input Each line of file contains | separated values as follow: <hostname>|<ip>|<groups>|<templates>")

    options,args = parser.parse_args()

    if not options.server:
        options.server = raw_input('server url:')
...
if __name__ == "__main__":
    ...
    if file:
        ...
        templates_id = zapi.template.get(output="templateid",filter={"host":templates.split(",")})
            ...
            print proxy_id
                    if proxy_id:
            print zapi.host.create({"host":hostname,"groups":groups_id,"templates":templates_id,"interfaces":[{"type":zabbix_type,"main":1,"useip":use_ip,"ip":ip,"dns":dns,"port":port}],"proxy_hostid":proxy_id,"status":status,"inventory_mode":inventory_mode})
            else:
            print zapi.host.create({"host":hostname,"groups":groups_id,"templates":templates_id,"interfaces":[{"type":zabbix_type,"main":1,"useip":use_ip,"ip":ip,"dns":dns,"port":port,"status":status,"inventory_mode":inventory_mode}]})
